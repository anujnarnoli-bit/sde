...
## Ephemeral public deploy (no account)
You can try an instant tunnel-based deploy from your terminal:

```bash
./scripts/deploy-localhostrun.sh
```

If your network allows SSH egress, this prints a temporary public URL from `localhost.run`.
The URL stays live only while that command keeps running.
